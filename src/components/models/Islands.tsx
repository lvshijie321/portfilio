...
import scene from "../../assets/3d/islands.glb";
...
interface IslandsProps {
  ...
  setCurrentStage: (num: number | undefined) => void;
}
...
const Islands: React.FC<IslandsProps> = ({
  ...
  const model: any = useGLTF(scene);
  const islandRef = useRef<any>();
  const { nodes, materials } = model;
  const three: any = useThree();
  const { gl, viewport } = three;
...
type CompareArray = number[];

const isEqualArray = (oldArray: CompareArray, newArray: CompareArray) => {
  const isEqual = oldArray.every((num: number, index: number) => {
    return newArray[index] === num;
  });
  return isEqual;
};

export default memo(Islands, (oldProps, newProps) => {
  return (
    isEqualArray(oldProps.scale, newProps.scale) &&
    oldProps.isRotating === newProps.isRotating &&
    oldProps.setIsRotating === oldProps.setIsRotating &&
    oldProps.setCurrentStage === oldProps.setCurrentStage
  );
});
